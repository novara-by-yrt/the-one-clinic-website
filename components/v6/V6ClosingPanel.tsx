...
import styles from './V6ClosingPanel.module.css';

/**
 * The last panel, which carries the closing call to action and the site
 * footer's content.
 *
 * The stage ends on this section rather than on a strip below the page,
 * so the site's own footer is suppressed on this route (see
 * components/layout/footer-mode.ts) and its content lives here instead: contact details, the address, the
...
export default function V6ClosingPanel({ id, section }: { id: string; section: number }) {
  return (
    <section
      id={id}
