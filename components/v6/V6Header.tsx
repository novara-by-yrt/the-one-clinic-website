...
import { isV6Route } from './v6-routes';
import './v6-tokens.css';
import styles from './V6Header.module.css';
 ...
 *
 * - Its gutters are the route's own gutter token (--v6-pad-x), so the
 ...
 *   left to right as the spreads flip. V6Stage publishes that as
 *   --v6-progress on the root element.
...
/** '#about' and '#conditions' are on-page anchors that no v6 panel has. */
...
export default function V6Header() {
  ...
  return (
    <div className={`${styles.shell} v6-chrome`} data-theme={open ? 'dark' : theme}>
      <div className={styles.bar}>
        ...
        {/* The rule is the reading position: it fills as the spreads
            flip. --v6-progress comes from V6Stage. */}
      ...
      {/* ── Contents ── */}
      <div id="v6-contents" className={styles.sheet} hidden={!open}>
        <div className={styles.sheetInner}>
          ...
          <div className={styles.sheetFoot}>
            ...
            <button type="button" className={`v6-cta ${styles.sheetCta}`} onClick={book}>
              Book a Consultation
            </button>
          </div>
        </div>
