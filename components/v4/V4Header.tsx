...
import { isV4Route } from './v4-routes';
import './v4-tokens.css';
import styles from './V4Header.module.css';
 ...
 *
 * - Its gutters are the route's own gutter token (--v4-pad-x), so the
 ...
 *   left to right as the spreads flip. V4Track publishes that as
 *   --v4-progress on the root element.
...
/** '#about' and '#conditions' are on-page anchors that no v4 panel has. */
...
export default function V4Header() {
  ...
  const active = isV4Route(pathname);
  ...
  return (
    <div className={`${styles.shell} v4-chrome`} data-theme={open ? 'dark' : theme}>
      <div className={styles.bar}>
        ...
        {/* The rule is the reading position: it fills as the spreads
            flip. --v4-progress comes from V4Track. */}
      ...
      {/* ── Contents ── */}
      <div id="v4-contents" className={styles.sheet} hidden={!open}>
        <div className={styles.sheetInner}>
          ...
          <div className={styles.sheetFoot}>
            ...
            <button type="button" className={`v4-cta ${styles.sheetCta}`} onClick={book}>
              Book a Consultation
            </button>
          </div>
        </div>
